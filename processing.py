...
def map_psms_to_spectra(spectra: List[Dict], psm_df: pd.DataFrame) -> pd.DataFrame:
    ...
    # ⚡ OPTIMIZATION: Use tolist() for faster iteration and explicit columns for faster DataFrame creation
    # Also pre-fills missing values with None to avoid NaN/float conversion for object columns
    target_cols = ['title', 'mz_array', 'intensity_array', 'pepmass']
    default_spec = {k: None for k in target_cols}

    # Convert matched Series to list using tolist() which is faster than iterating over Series
    specs_list = [x if isinstance(x, dict) else default_spec for x in matched_spec_series.tolist()]

    # Create DataFrame with explicit columns (faster than inference)
    specs_df = pd.DataFrame(specs_list, columns=target_cols)
